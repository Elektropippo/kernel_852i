...
#if 1
#define dprint(format, x...) printk("Elektro: "format, ## x)
#else
...
#if 1
#define rk28printk(format, x...) printk("Elektro:"format, ## x)
#else
#define rk28printk(x...)
#endif
...
static const char *version = "Elektro mma7660";
/* AKM HW info */
...
rk28printk("Elektro mma7660_report_value now call!!!!\n" );
	    mma7660_report_value(client, &axis);
...
static int mma7660_suspend( struct early_suspend *h)
{
   //struct i2c_client *client = container_of(mma7660_device.parent, struct i2c_client, dev);
   
   dprint("Gsensor mma7760 enter suspend\n");
   return mma7660_close_dev( g_client );
}

static int mma7660_resume( struct early_suspend *h)
{
   //struct i2c_client *client = container_of(mma7660_device.parent, struct i2c_client, dev);
    //struct mma7660_data *mma7660 = (struct mma7660_data *)i2c_get_clientdata(client);
    struct mma7660_data *mma7660 = (struct mma7660_data *)i2c_get_clientdata( g_client );
   dprint("Gsensor mma7760 resume!!\n");
   //return mma7660_start_dev(client,mma7660->curr_tate);
   return mma7660_start_dev( g_client, mma7660->curr_tate );
}
*/
#else
...
//#if !defined(CONFIG_ANDROID_POWER) && !defined(CONFIG_HAS_EARLYSUSPEND)
...
static void __init mma7660_init_async(void *unused, async_cookie_t cookie)
{
   //printk("--------> %s <-------------\n",__func__);
	dprint("Elektro l=%d,%s:\n",__LINE__, __func__);
...
{
	dprint("Elektro l=%d,%s:\n",__LINE__, __func__);
	//async_schedule(mma7660_init_async, NULL);
