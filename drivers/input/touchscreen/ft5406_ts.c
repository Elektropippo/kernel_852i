...
//aggiunto
		for(i=0;i<MAX_POINT;i++)
		{
			...
			{
				//printk("[TSP] Point UP event.id=%d\n",i);
...
//aggiunto
	for(i=0;i<MAX_POINT;i++)
	{
		  if((current_events[i].flag == 0) && (points_last_flag[i] != 0))
			...
			{	
		  		//printk("[TSP] Point DN event.id=%d, posx=%d, posy=%d\n",i,current_events[i].x,current_events[i].y);
					input_mt_slot(data->input_dev, i);
...
//	input_set_abs_params(input_dev,ABS_MT_WIDTH_MAJOR, 0, WIDTH_MAJOR_MAX, 0, 0);
	...
	/***wait CTP to bootup normally***/
	 ...
	 printk("[TSP] firmware version = 0x%2x local version X%2x\n", reg_version,fts_ctpm_get_upg_ver());
